


import 'package:flutter/foundation.dart';
import 'package:grabto/model/filtered_data_model.dart';
import 'package:grabto/model/get_post_model.dart';
import 'package:grabto/services/api.dart';

import '../helper/network/base_api_services.dart';
import '../helper/network/network_api_services.dart';

class GetPostRepo {
  final BaseApiServices _apiServices = NetworkApiServices();

  Future<GetPostModel> getPostApi(dynamic data) async {
    try {
      print("QQQQ$data");
      dynamic response =
      await _apiServices.getPostApiResponse('$BASE_URL/get-user-post',data );
      return GetPostModel.fromJson(response);
    } catch (e) {
      if (kDebugMode) {
        print('Error occurred during getPostApi: $e');
      }
      rethrow;
    }
  }

}